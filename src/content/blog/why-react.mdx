...
しかし、データに応じてDOMの操作を開発者自身が担うという問題は残っている。
アプリケーションの規模が大きくなるほど保守が難しくなるという問題は残ったままだ。
...
これにより、データの変更とDOMの状態を同期する処理の一部をReact側に任せられるようになった、と筆者は捉えている。
...
他にも色々書いてあるがReactの良さが滲み出ているのは状態とUIが近しい関係にある設計がされていることだと感じた。
...
- Reactが生まれる前のUI構築は開発者が直接DOMを操作してUIを更新する方法が一般的だった
...
Reactは現在多くのユーザーに使われ続け進化を続けている。今では**RSC**や**Async React**などの新しい概念が生れている。

ただ、どのような方向へ進化しても、Reactの強みの一つは「現在の状態に応じてUIが決まる」という考え方にあると筆者は考えている。

Reactが生まれる前は、データの変更に応じてどのDOMをどのように変更するかを開発者自身が管理する必要があった。Reactはその責任の一部を引き受け、状態とUIの関係を宣言的に記述できるようにした。
